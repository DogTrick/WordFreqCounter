import Data.List as L (foldr, map, sortOn)
import Data.Set as S (Set, fromList, member)
import Data.Map as M (Map, lookup, insert, empty, keys)
import Data.Char (toLower)

data Trie = Node Int (M.Map Char Trie)
    deriving Show

splitChars :: S.Set Char
splitChars = S.fromList [' ', ',', '.', '!', '?', ';', ':', '\n', '/', '\\', '-', '_', '"', '(', ')', '[', ']', '{', '}', '<', '>', '&', '~', '|', '#', '@', '`']
...
contextToTrie = aux (Node 0 M.empty)
    where popWord y "" = (reverse y, "")
          popWord y (x:xs) = if x `member` splitChars then (reverse y, xs)
